...
@Ignore
public class FreemarkerStepMetaTest {
  ...
  public void testGetStepData() {
    FreemarkerStepMeta m = new FreemarkerStepMeta();
    assertEquals( FreemarkerStepData.class, m.getStepData().getClass() );
  }
  ...
  public void testStepAnnotations() {
    ...
    Step stepAnnotation = FreemarkerStepMeta.class.getAnnotation( Step.class );
    ...
    assertEquals( FreemarkerStepMeta.class.getPackage().getName(), stepAnnotation.i18nPackageName() );
    ...
    InjectionSupported injectSupportAnnotation = FreemarkerStepMeta.class.getAnnotation( InjectionSupported.class );
    ...
    for ( Field field : FreemarkerStepMeta.class.getDeclaredFields() ) {
      if ( field.isAnnotationPresent( Injection.class ) || field.isAnnotationPresent( InjectionDeep.class ) ) {
        hasInjection = true;
      }
    }
    for ( Method method : FreemarkerStepMeta.class.getDeclaredMethods() ) {
  ...
  public void testLoadSave() throws KettleException {
     ...
     */
    LoadSaveTester<FreemarkerStepMeta> tester =
      new LoadSaveTester<FreemarkerStepMeta>( FreemarkerStepMeta.class, attributes );
  ...
  public void testChecks() {
    ...
    for ( CheckResultInterface result : checkResults ) {
      if ( result.getType() == CheckResultInterface.TYPE_RESULT_ERROR
          && result.getText().equals( BaseMessages.getString( FreemarkerStepMeta.class, "Demo.CheckResult.ReceivingRows.ERROR" ) ) ) {
        foundMatch = true;
      }
    }
    ...
    for ( CheckResultInterface result : checkResults ) {
      if ( result.getType() == CheckResultInterface.TYPE_RESULT_OK
          && result.getText().equals( BaseMessages.getString( FreemarkerStepMeta.class, "Demo.CheckResult.ReceivingRows.OK" ) ) ) {
        foundMatch = true;
      }
    }
    assertTrue( "The step checks should fail if no input fields are given", foundMatch );
